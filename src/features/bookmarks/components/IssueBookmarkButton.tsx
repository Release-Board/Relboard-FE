...
export default function IssueBookmarkButton({ issueId }: Props) {
  const router = useRouter();
  const { user, accessToken } = useAuthStore();
  const { t } = useStableTranslation();
  const { isBookmarked, addBookmark, removeBookmark } = useIssueBookmarks({
    page: 0,
    size: 200,
  });
